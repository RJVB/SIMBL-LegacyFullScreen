...
BOOL _ZKSwizzle(Class src, Class dest) {
    ...
    NSString *srcName = NSStringFromClass(src);
    ...
    if ([classTable objectForKey:srcName]) {
        [NSException raise:@"Invalid Argument"
                    format:@"This source class (%@) was already swizzled with another, (%@)", srcName, classTable[srcName]];
        return NO;
    }
    ...
    [classTable setObject:destName forKey:srcName];
    return success;
}
...
//                NSLog(@"%s:%d - addMethod %@ %@", __PRETTY_FUNCTION__, __LINE__, destination, NSStringFromSelector(selector));
...
//                NSLog(@"%s:%d - addMethod %@ %@", __PRETTY_FUNCTION__, __LINE__, destination, NSStringFromSelector(destSel));
...
//            NSLog(@"%s:%d - addMethod %@ %@ %s", __PRETTY_FUNCTION__, __LINE__, destination, NSStringFromSelector(selector), method_getTypeEncoding(method));
            success &= class_addMethod(destination, selector, method_getImplementation(method), method_getTypeEncoding(method));
        }
    }
